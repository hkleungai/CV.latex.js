...
all: pdftex bibtex src/texlive.lst
...
src/pdftex-worker.js: pdftex-pre.js pdftex-post.js pdftex.bc
	...
	#OBJFILES=$$(for i in `find build/texk/web2c/lib build/texk/kpathsea -name '*.o'` ; do llvm-nm $$i | grep main >/dev/null || echo $$i ; done) && \
		emcc  --memory-init-file 0 -v --closure 1 -s TOTAL_MEMORY=$$((128*1024*1024)) -O3  pdftex.bc -s INVOKE_RUN=0 --pre-js pdftex-pre.js --post-js pdftex-post.js -o src/pdftex-worker.js -s ERROR_ON_UNDEFINED_SYMBOLS=0 -s SINGLE_FILE=1 -g1
.PHONY: pdftex
pdftex: src/pdftex-worker.js
...
src/texlive.lst: texlive
	find texlive -type d -exec echo {}/. \; | sed 's/^texlive//g' >src/texlive.lst
	find texlive -type f | sed 's/^texlive//g' >>src/texlive.lst
...
clean:
	...
	rm -f src/texlive.lst
	rm -f src/pdftex-worker.js
